...
for line in ['线路6','线路11']:
    ...
    cardtype_ratio = pd.read_csv(cardtype_name)
    # print gd_train_line_pd_weather.columns
    gd_train_line_pd_weather = pd.merge(gd_train_line_pd_weather,cardtype_ratio, on=['date','time'])
    ...
    parameters={'loss':('ls', 'lad', 'huber', 'quantile'),'learning_rate':[0.04*(i+1) for i in range(25)],
                'n_estimators':[125,150,200,250],'max_depth':[3,4,5,6]}
